...
export function FileUpload({ onFileSelect, disabled, uploadedImage, onRemove }: FileUploadProps) {
  ...
  if (uploadedImage) {
    return (
      <div className={`${theme.panel} relative rounded-3xl p-6`}
        aria-live="polite"
      >
        ...
        <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className={`${theme.subheading} text-sm`}>Image ready. Replace it to choose another file.</div>
          <label
            htmlFor="file-input-replace"
            className={`${theme.accentButton} cursor-pointer rounded-xl px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-purple-900/40 transition-transform duration-200 hover:scale-[1.01] ${
